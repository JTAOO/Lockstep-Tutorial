...
namespace Lockstep.Game
{
    ...
    public class RoomMsgManager : IRoomMsgManager
    {
        ...
        private IRoomMsgHandler _handler; // MessageHandler, 用来把对应的网络数据转换到记录好的那些事件函数
        ...
        {
            _maxMsgId = (byte)System.Math.Min((int)EMsgSC.EnumCount, (int)byte.MaxValue); // 最大函数ID (一共才48个吗???)
            ...
            _netTcp.DoStart(); // 连接服务器
            _netTcp.NetMsgHandler = OnNetMsg; // 网络消息解析器
        }
